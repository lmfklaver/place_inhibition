function [sleep_FR_mat, exp_FR_mat] = getFR_BoxPlot_SingleSessoin_PlaceInhibition(basePath, sleepQual)
% PURPOSE
...
%          and 3 firing rate experimental box plots for a single recording
%          session.
%
% INPUTS
...
%          Two figures
%              - Box plot of firing rate during sleep segments
%              - Box plot of firing rate during experimental segments
% DEPENDENCIES
...
%% Plotting
    subplot(1,2,1);
    boxplot(sleep_FR_mat, [X1(1) X2(1) X3(1) X4(1)]);
    hold on;
    ...
    subplot(1,2,2);
    boxplot(exp_FR_mat, [X1(1) X2(1) X3(1)]);
    hold on;
