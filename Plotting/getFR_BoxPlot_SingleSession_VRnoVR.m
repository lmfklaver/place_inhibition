function [sleep_FR_mat, exp_FR_mat] = getFR_BoxPlot_SingleSession_VRnoVR(basePath, sleepQual)
% PURPOSE
...
%          and 2 firing rate experimental (VR and no VR) box plots.
%
% INPUTS
%          basePath    String    : Location of data
...
%% Find firing rate for each sleep and experimental segment, and boxplot it      
       ...
       % find how many spikes there are in each segment;
             sleep_FR_mat = zeros(length(spikes.times),2);
             exp_FR_mat = zeros(length(spikes.times),2);
...
figure;
    subplot(1,2,1);
    boxplot(sleep_FR_mat, [X1(1) X2(1)]);
    hold on;
    h = findobj(gca,'Tag','Box');
    for j=1:length(h)
        patch(get(h(j),'XData'),get(h(j),'YData'),color_sleep(j,:),'FaceAlpha',.5);
    end
    legend('Sleep 1','Sleep 2');
    set(gca,'xticklabel',[]);
    title('Firing Rate For Sleep Sessions');
    ylabel('Firing Rate (spikes/s)');
    set(gca, 'xdir','reverse');
    hold off;
    
    subplot(1,2,2);
    boxplot(exp_FR_mat, [X1(1) X2(1)]);
    hold on;
    h2 = findobj(gca,'Tag','Box');
    for j=1:length(h2)
        patch(get(h2(j),'XData'),get(h2(j),'YData'),exp_colors(j,:),'FaceAlpha',.5);
    end
    legend('VR','no VR');
    title('Firing Rate For Experimental Sessions');
    ylabel('Firing Rate (spikes/s)');
    set(gca,'xticklabel',[]);
    set(gca, 'xdir','reverse');
end